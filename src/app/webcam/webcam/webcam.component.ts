...
export class WebcamComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   */
  private static getDeviceIdFromMediaStreamTrack(mediaStreamTrack: MediaStreamTrack): any {
   ...
   */
  private static getFacingModeFromMediaStreamTrack(mediaStreamTrack: MediaStreamTrack): any {
    if (mediaStreamTrack) {
  ...
  private detect = async (net) => {
    if (
      typeof this.nativeVideoElement !== "undefined" &&
      ...
      this.drawCanvas(pose, video, video.width, video.height, this.canvas.nativeElement);
    }
  };

  private drawCanvas = (pose, video, videoWidth, videoHeight, canvas) => {
